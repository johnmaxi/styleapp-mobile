...
type Commission = {
  id: number; service_id: number; total_service: number;
  commission_amt: number; professional_amt: number;
  payment_method: string; payment_status: string;
  notes?: string; created_at: string;
  professional_name?: string; professional_role?: string;
};

type ByProfRow = {
  barber_id: number; barber_name?: string;
  completed_total?: number; commission_total?: number; revenue_total?: number;
};

type Totals = {
  total_transacciones: number; total_servicios: number;
  saldo_app: number; total_profesionales: number;
};

type Professional = {
  id: number; name: string; email: string; role: string;
  phone?: string; document_type?: string; document_number?: string;
  id_front?: string; id_back?: string; diploma?: string;
  antecedentes_doc?: string; profile_photo?: string;
  created_at: string; registration_status: string;
};

type Tab = "registros" | "comisiones";

const ROLE_LABELS: Record<string, string> = {
  barber: "Barbero", estilista: "Estilista", quiropodologo: "Quiropodólogo",
};

const PAYMENT_LABELS: Record<string, string> = {
  efectivo: "💵 Efectivo", nequi: "📱 Nequi",
  pse: "🏦 PSE", tarjeta: "💳 Tarjeta",
};
...
export default function AdminScreen() {
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            setActing(true);
            try {
              await api.post(`/admin/review-professional/${prof.id}`, { action: "approve" });
  ...
  const openDoc = (url?: string) => {
    ...
    if (url.includes(".pdf") || url.startsWith("data:application/pdf")) {
      Alert.alert("Documento PDF", "El documento PDF fue adjuntado correctamente al registro.\n\nPara visualizarlo descárgalo desde el enlace adjunto.", [{ text: "OK" }]);
      return;
    }
    setImageModal(url);
  };
  ...
  return (
    <ScrollView contentContainerStyle={{
      padding: 20, gap: 14, backgroundColor: palette.background, paddingBottom: 40,
    }}>
      {/* Header */}
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
        ...
        >
          <Text style={{ color: "#dd0000", fontWeight: "700", fontSize: 13 }}>Salir</Text>
        </TouchableOpacity>
      </View>
        ...
        <>
          {professionals.length === 0 ? (
            <View style={{ backgroundColor: palette.card, padding: 24,
              ...
              <Text style={{ color: "#888", fontSize: 16 }}>✅ No hay registros pendientes</Text>
            </View>
          ) : (
            professionals.map((prof) => (
              <View key={prof.id} style={{
                ...
                <View style={{ gap: 3, marginBottom: 12 }}>
                  {prof.phone && <Text style={{ color: "#aaa", fontSize: 13 }}>📞 {prof.phone}</Text>}
                  {prof.document_type && (
      ...
      {tab === "comisiones" && (
        <>
          ...
          {totals && (
            <View style={{
              ...
              <Text style={{ color: "#888", fontSize: 12 }}>
                {totals.total_transacciones} transacciones •{" "}
          ...
          {/* Filtro fechas */}
          <View style={{ backgroundColor: palette.card, borderRadius: 12, padding: 14, gap: 10 }}>
            <Text style={{ color: palette.primary, fontWeight: "700" }}>🗓 Filtrar por fechas</Text>
            <View style={{ flexDirection: "row", gap: 10 }}>
              <View style={{ flex: 1 }}>
                ...
                <TextInput
                  ...
                  style={{ backgroundColor: "#1a1a1a", borderWidth: 1,
                    borderColor: palette.primary + "55", borderRadius: 8,
                    padding: 10, color: "#fff", fontSize: 13 }}
                />
              </View>
              <View style={{ flex: 1 }}>
          ...
          {/* Totales período */}
          {filteredTotals && (
            <View style={{ backgroundColor: palette.card, borderRadius: 12,
              ...
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10 }}>
                {[
                  { label: "Transacciones",      value: String(filteredTotals.total_transacciones) },
                  { label: "Total servicios",     value: `$${Number(filteredTotals.total_servicios).toLocaleString("es-CO")}` },
                  ...
                  <View key={label} style={{ backgroundColor: "#1a1a1a",
                    borderRadius: 10, padding: 12, minWidth: "45%", flex: 1 }}>
                    <Text style={{ color: "#888", fontSize: 11 }}>{label}</Text>
                    <Text style={{ color: palette.primary, fontWeight: "700", fontSize: 16 }}>{value}</Text>
                  </View>
                ))}
              </View>
            </View>
          ...
          {byProf.length > 0 && (
            <>
              <Text style={{ color: palette.text, fontWeight: "700" }}>Por profesional</Text>
              {byProf.map((row) => (
                <View key={row.barber_id} style={{
                  ...
                  <Text style={{ color: "#aaa", fontSize: 12 }}>Servicios: {row.completed_total || 0}</Text>
          ...
          <TouchableOpacity
            ...
            style={{ backgroundColor: "#0d1b2e", borderWidth: 1,
          ...
          {showHistory && history.map((c) => (
            <View key={c.id} style={{
              ...
              <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                <Text style={{ color: "#aaa", fontSize: 11 }}>Servicio #{c.service_id}</Text>
                <Text style={{ color: "#666", fontSize: 11 }}>{fmtDate(c.created_at)}</Text>
              </View>
              <Text style={{ color: palette.text, fontWeight: "700" }}>
                {c.professional_name || "—"}
              ...
              <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                <Text style={{ color: "#4caf50", fontSize: 12 }}>
                  Comisión: ${Number(c.commission_amt).toLocaleString("es-CO")}
                </Text>
                <Text style={{ color: "#888", fontSize: 12 }}>
      ...
      <TouchableOpacity
        ...
        style={{ borderWidth: 1, borderColor: "#555",
        ...
        <View style={{ flex: 1, justifyContent: "flex-end", backgroundColor: "#00000099" }}>
          <View style={{ backgroundColor: "#1a1a1a", borderTopLeftRadius: 20,
            ...
            <TextInput
              ...
              style={{ backgroundColor: "#0d0d0d", borderWidth: 1,
            ...
            <TouchableOpacity
              ...
              style={{ backgroundColor: acting || !rejectReason.trim() ? "#333" : "#dd0000",
      ...
      <Modal visible={!!imageModal} transparent animationType="fade">
        ...
        >
          {imageModal && (
            <Image source={{ uri: imageModal }}
              style={{ width: "92%", height: "75%", borderRadius: 12 }}
              resizeMode="contain" />
          )}
          <Text style={{ color: "#888", marginTop: 16, fontSize: 13 }}>Toca para cerrar</Text>
        </TouchableOpacity>
      </Modal>
    </ScrollView>
  );
