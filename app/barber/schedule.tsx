...
export default function BarberSchedule() {
  ...
  const hasServices = (date: Date) => {
    return getServicesForDay(date).length > 0 || getCompletedForDay(date).length > 0;
  };
  ...
  const goToService = (svc: ScheduledService) => {
    if (svc.status === "accepted" || svc.status === "on_route" || svc.status === "arrived") {
      router.push({
      ...
      {selectedDay && !loading && (
        <View style={{ gap: 10 }}>
          ...
          {selectedServices.map(svc => {
            ...
            const isActive  = ["accepted","on_route","arrived"].includes(svc.status || "");
            ...
            <>
              ...
              {selectedCompleted.map(svc => (
                <View key={svc.id} style={{
                  ...
                  borderWidth: 1, borderColor: "#333", gap: 4, opacity: 0.7,
                }}>
                  <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                    ...
                    <Text style={{ color: "#555", fontSize: 11 }}>
                      {svc.completed_at
                        ? new Date(svc.completed_at).toLocaleTimeString("es-CO",
                            { hour: "2-digit", minute: "2-digit" })
                        : ""}
                    </Text>
                  </View>
                  <Text style={{ color: "#888" }}>{svc.service_type || "Servicio"}</Text>
                  {svc.client_name && (
                    <Text style={{ color: "#666", fontSize: 12 }}>👤 {svc.client_name}</Text>
                  )}
          ...
          {scheduled.length > 0 && (
            <View style={{ backgroundColor: "#0d1b2e", borderRadius: 10, padding: 14,
              ...
              <Text style={{ color: "#2196F3", fontWeight: "700", marginBottom: 8 }}>
                📅 Esta semana — {scheduled.length} servicio{scheduled.length !== 1 ? "s" : ""} programado{scheduled.length !== 1 ? "s" : ""}
              </Text>
              {scheduled.slice(0, 3).map(svc => (
                <View key={svc.id} style={{ borderBottomWidth: 1, borderBottomColor: "#1a2a3a",
